"""FSM States for expense entry flow."""
from aiogram.fsm.state import State, StatesGroup


class ExpenseStates(StatesGroup):
    """States for expense entry wizard."""
    waiting_amount = State()         # Step 1: Enter amount
    waiting_category = State()       # Step 2: Select expense category
    waiting_new_category = State()   # Step 2b: Enter new category name
    waiting_payment_method = State() # Step 3: Select payment method
    waiting_description = State()    # Step 4: Description (optional)
